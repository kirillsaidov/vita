...
int64_t algorithm_search_linear_ptr(const void* ptr, const void** list, const size_t length) {
...
int64_t algorithm_search_linear_int(const int value, const int* array, const size_t length) {
	// search for an element iteratively
	for(size_t i = 0; i < length; i++) {
		if(array[i] == value) {
			return i;
