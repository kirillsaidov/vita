...
int32_t main(void) {
    ...
    const vt_str_t static_s = str_make_on_stack("hello, world");
    assert(str_equals(vt_cstr(&static_s), "hello, world"));
    assert(str_starts_with(vt_cstr(&static_s), "hello"));
    assert(str_ends_with(vt_cstr(&static_s), "world"));

    vt_str_t* mystr = str("hello, world!"); {
        assert(str_len(mystr) == 13);
        assert(str_capacity(mystr) == 13);
        assert(str_has_space(mystr) == 0);
        assert(!str_is_empty(mystr));
        assert(str_equals(vt_cstr(mystr), "hello, world!"));
        ...
        assert(str_equals(vt_cstr(mystr), "Apples are sweet!"));
        ...
        assert(str_equals(vt_cstr(mystr), "Oran.Apples are sweet!?"));

        const vt_str_t sstack = str_make_on_stack("hi this is a test hi, world hi");
        ...
        assert(str_equals(vt_cstr(mystr), ""));

        vt_str_t *copy2 = strn(10); {
    ...
    vt_str_t *ns = strn(0); {
        ...
        assert(str_append(ns, "hello") == vt_ve_operation_success);
        assert(str_equals(vt_cstr(ns), "hello"));

        // fails, because str_len(ns) < strlen("hello, world") => append instead
        // assert(str_set(ns, "hello, world"));
    } str_free(ns);

    vt_str_t *sto = str_take_ownership(strdup("12345, world")); {
        assert(str_len(sto) == 12);
        assert(str_capacity(sto) == 12);
        assert(str_has_space(sto) == 0);

        assert(str_append(sto, "! How are you?") == vt_ve_operation_success);
        assert(str_len(sto) == strlen("12345, world! How are you?"));
        assert(str_equals(vt_cstr(sto), "12345, world! How are you?"));
        assert(str_append(sto, " hello, world; hello again. This is hello!") == vt_ve_operation_success);

        str_clear(sto);
        str_append(sto, ";My name is Kirillos;How are you?;let's play;");
        vt_plist_t *p = str_split(NULL, sto, ";"); {
        ...
        vt_str_t *spf = vt_str_pop_get_first(NULL, sto, "Kirillos"); {
            assert(str_equals(vt_cstr(spf), ";My name is "));
        } str_free(spf);

        vt_str_t *spl = str_pop_get_last(NULL, sto, "you?;"); {
            assert(str_equals(vt_cstr(spl), "let's play;"));
        } str_free(spl);

        assert(str_equals(vt_cstr(sto), ";How are "));

        vt_str_t *_s__ = str("hello world of world of wonders!"); {
            vt_str_t *_s_ = str_pop_get_last(NULL, _s__, "world"); {
                assert(str_equals(vt_cstr(_s_), " of wonders!"));
            } str_free(_s_);
            
            assert(str_equals(vt_cstr(_s__), "hello world of "));
        } str_free(_s__);

    } str_free(sto);

    vt_str_t *s_strip = str("   \nhello, world\t  ");
    str_strip(s_strip);
    assert(str_len(s_strip) == 12);
    assert(str_equals(vt_cstr(s_strip), "hello, world"));
    str_free(s_strip);
    ...
    assert(str_equals(vt_cstr(s_strip), "helloworld"));
    str_free(s_strip);
    
    s_strip = str("Here is a shopping list: apples and oranges, milk and sugar, and vinegar.");
    str_remove_all(s_strip, " and");
    assert(str_equals(vt_cstr(s_strip), "Here is a shopping list: apples oranges, milk sugar, vinegar."));
    str_free(s_strip);
    ...
    assert(str_equals(vt_cstr(s_strip), "bbbbb"));
    str_clear(s_strip);

    str_appendf(s_strip, "%s %d %s", "I have", 2, "apples");
    assert(str_equals(vt_cstr(s_strip), "I have 2 apples"));
    str_free(s_strip);

    s_strip = str_fmt(NULL, "%s, %s. %s %d!", "hello", "world", "hi", 5);
    assert(str_equals(vt_cstr(s_strip), "hello, world. hi 5!"));
    str_free(s_strip);
