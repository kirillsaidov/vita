...
int32_t main(void) {
    ...
    vt_str_t* mystr = vt_str_create("hello, world!", alloctr); {
        ...
        assert(vt_str_equals_z(vt_str_z(mystr), "hello, world!"));
        ...
        assert(vt_str_equals_z(vt_str_z(mystr), "Apples are sweet!"));
        ...
        assert(vt_str_equals_z(vt_str_z(mystr), ""));
    ...
    vt_str_t *tmp_str = vt_str_create("<defs></defs>", alloctr); {
        ...
        assert(vt_str_equals_z(vt_str_z(tmp_str), "<defs>[]</defs>"));

        vt_str_insert_n(tmp_str, "123456789", 7, 3);
        assert(vt_str_equals_z(vt_str_z(tmp_str), "<defs>[123]</defs>"));
        ...
        assert(vt_str_equals_z(vt_str_z(tmp_str), "<defs>[[123]][[456]]</defs>"));

        vt_str_insert_n(tmp_str, "1234567890", 13, 3);
        assert(vt_str_equals_z(vt_str_z(tmp_str), "<defs>[[123]]123[[456]]</defs>"));

        vt_str_insert_n(tmp_str, "4567890", 16, 3);
        assert(vt_str_equals_z(vt_str_z(tmp_str), "<defs>[[123]]123456[[456]]</defs>"));

        vt_str_insert_n(tmp_str, "7890", 19, 3);
        assert(vt_str_equals_z(vt_str_z(tmp_str), "<defs>[[123]]123456789[[456]]</defs>"));

        vt_str_clear(tmp_str);
        vt_str_append(tmp_str, "hello ");
        vt_str_appendf(tmp_str, "world ");
        vt_str_append(tmp_str, "APPEND");
        assert(vt_str_equals_z(vt_str_z(tmp_str), "hello world APPEND"));

        vt_str_insert_before(tmp_str, "world", "[1, 2, 3] ");
        assert(vt_str_equals_z(vt_str_z(tmp_str), "hello [1, 2, 3] world APPEND"));
    ...
    vt_str_t *sto = vt_str_take_ownership(strdup("12345, world"), alloctr); {
        ...
        assert(vt_str_equals_z(vt_str_z(sto), "12345, world! How are you?"));
        ...
        vt_str_t *spf = vt_str_pop_get_first(NULL, sto, "Kirillos"); {
            assert(vt_str_equals_z(vt_str_z(spf), ";My name is "));
        } vt_str_destroy(spf);

        vt_str_t *spl = vt_str_pop_get_last(NULL, sto, "you?;"); {
            assert(vt_str_equals_z(vt_str_z(spl), "let's play;"));
        } vt_str_destroy(spl);

        assert(vt_str_equals_z(vt_str_z(sto), ";How are "));

        vt_str_t *_s__ = vt_str_create("hello world of world of wonders!", alloctr); {
            vt_str_t *_s_ = vt_str_pop_get_last(NULL, _s__, "world"); {
                assert(vt_str_equals_z(vt_str_z(_s_), " of wonders!"));
            } vt_str_destroy(_s_);
            
            assert(vt_str_equals_z(vt_str_z(_s__), "hello world of "));
        } vt_str_destroy(_s__);
    ...
    assert(vt_str_equals_z(vt_str_z(s_strip), "helloworld"));
    vt_str_destroy(s_strip);
    ...
    assert(vt_str_equals_z(vt_str_z(s_strip), "Here is a shopping list: apples oranges, milk sugar, vinegar."));
    vt_str_destroy(s_strip);
    ...
    vt_str_ensure_len(sbetween, 12);
    assert(vt_str_len(sbetween) == 12);
    vt_str_ensure_capacity(sbetween, 21);
    assert(vt_str_capacity(sbetween) == 21);
    ...
    {
        assert(vt_str_pop_get_first(NULL, new_s, "/") == NULL);
        assert(vt_str_pop_get_last(NULL, new_s, "/") == NULL);
        assert(vt_str_pop_get_first(NULL, new_s, "file.txt") == NULL);
        assert(vt_str_pop_get_last(NULL, new_s, "file.txt") == NULL);
        assert(vt_str_pop_get_first(new_s, new_s, "file.txt") == new_s);
        assert(vt_str_pop_get_last(new_s, new_s, "file.txt") == new_s);
    }
    vt_str_destroy(new_s);

    vt_str_t *s1 = vt_str_create("John", alloctr); 
    {
        vt_str_t *s2 = vt_str_dup(s1);
        assert(vt_str_equals(s1, s2));
        vt_str_destroy(s2);
    }
    vt_str_destroy(s1);
